// ExplosionTests — gate do contrato público de explosão (agente 4 §5 item
// 58, componente CORE). Prova o falloff determinístico (1 − d/radius)^power
// com clamp, a validação all-or-nothing do spec e os fragmentos
// determinísticos por seed (direção unitária, speed no range, massa
// uniforme, mesmo seed → bit-exact).
...
int main() {
    ...
    if (g_failures == 0) {
        std::cout << "explosion_tests: all checks passed\n";
        return 0;
    }
    std::cout << "explosion_tests: " << g_failures << " failure(s)\n";
    return 1;
}
