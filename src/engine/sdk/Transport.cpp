...
class Loopback final:public ITransport{
 TransportConfig cfg_;TransportEndpoint local_,remote_;TransportState state_{TransportState::Stopped};std::deque<TransportMessage> q_;std::uint64_t next_{1},sent_{0},received_{0},peer_{0};
public:
 explicit Loopback(TransportConfig c):cfg_(std::move(c)),local_(cfg_.endpoint){}
 bool start(const TransportConfig&c,std::string&e)override{if(c.max_message_bytes==0||c.poll_budget==0){e="invalid_transport_config";return false;}cfg_=c;local_=c.endpoint;q_.clear();next_=1;sent_=received_=0;state_=TransportState::Listening;return true;}
