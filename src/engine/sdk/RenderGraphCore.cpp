// RenderGraphCore.cpp — Agente 1 (task_plan B2): the PUBLIC Render Graph
// adapter. Faithful, deterministic port of the internal RenderGraph compile
// algorithm (src/engine/rendering/RenderGraph.cpp): DAG of resources/passes,
// explicit dependencies + resource-hazard edges, topological order with
// declaration-order tiebreaks, barriers on state/access/queue transitions,
// resource lifetimes. Self-contained (std only) — the Vulkan executor is the
// seam that consumes this model.
//
// Equivalence with the internal implementation is proven by the gate
// (RenderGraphCoreTests.cpp compiles BOTH and asserts bit-exact identical
// results for identical graphs).
...
class RenderGraphCore final : public IRenderGraph {
public:
    RenderGraphCore() = default;

    RenderResourceId add_resource(RenderResourceDesc resourceDesc) override {
    ...
    RenderPassId add_pass(RenderPassDesc passDesc) override {
    ...
    bool add_dependency(RenderPassId before, RenderPassId after) override {
    ...
    bool remove_pass(RenderPassId id) override {
        const auto it = std::find(passIds_.begin(), passIds_.end(), id);
        if (it == passIds_.end()) return false;
        const std::size_t index = static_cast<std::size_t>(std::distance(passIds_.begin(), it));
        passIds_.erase(it);
    ...
    bool remove_resource(RenderResourceId id) override {
        ...
        for (const auto& passDesc : passes_)
            if (std::any_of(passDesc.resources.begin(), passDesc.resources.end(), [id](const auto& access) { return access.resource == id; }))
                return false;
        const std::size_t index = static_cast<std::size_t>(std::distance(resourceIds_.begin(), it));
        resourceIds_.erase(it);
        resources_.erase(resources_.begin() + static_cast<std::ptrdiff_t>(index));
        return true;
    }

    bool set_pass_enabled(RenderPassId id, bool enabled) override {
        const auto it = std::find(passIds_.begin(), passIds_.end(), id);
        if (it == passIds_.end()) return false;
        passes_[static_cast<std::size_t>(std::distance(passIds_.begin(), it))].enabled = enabled;
        return true;
    }

    const RenderResourceDesc* resource(RenderResourceId id) const noexcept override {
        const auto it = std::find(resourceIds_.begin(), resourceIds_.end(), id);
        return it == resourceIds_.end() ? nullptr : &resources_[static_cast<std::size_t>(std::distance(resourceIds_.begin(), it))];
    }

    const RenderPassDesc* pass(RenderPassId id) const noexcept override {
        const auto it = std::find(passIds_.begin(), passIds_.end(), id);
        return it == passIds_.end() ? nullptr : &passes_[static_cast<std::size_t>(std::distance(passIds_.begin(), it))];
    }

    const std::vector<RenderResourceDesc>& resources() const noexcept override { return resources_; }
    const std::vector<RenderPassDesc>& passes() const noexcept override { return passes_; }
    const std::vector<RenderResourceId>& resource_ids() const noexcept override { return resourceIds_; }
    const std::vector<RenderPassId>& pass_ids() const noexcept override { return passIds_; }

    RenderGraphCompileResult compile() const override {
        RenderGraphCompileResult result;
        std::unordered_map<RenderPassId, std::size_t> passIndex;
        std::unordered_map<RenderResourceId, std::size_t> resourceIndex;
        for (std::size_t i = 0; i < passIds_.size(); ++i) passIndex.emplace(passIds_[i], i);
        for (std::size_t i = 0; i < resourceIds_.size(); ++i) resourceIndex.emplace(resourceIds_[i], i);
        ...
        for (std::size_t left = 0; left < passes_.size(); ++left) {
            if (!passes_[left].enabled) continue;
            for (std::size_t right = left + 1; right < passes_.size(); ++right) {
        ...
        std::unordered_map<RenderPassId, std::uint32_t> indegree;
        ...
        using Candidate = std::pair<std::size_t, RenderPassId>;
        ...
        for (RenderResourceId id : resourceIds_) {
            ...
            lifetimes.emplace(id, RenderResourceLifetime{id, std::numeric_limits<std::uint32_t>::max(), 0, desc->transient});
        }

        for (std::uint32_t compiledIndex = 0; compiledIndex < result.order.size(); ++compiledIndex) {
        ...
        for (RenderResourceId id : resourceIds_) {
            ...
            if (lifetime.firstUse != std::numeric_limits<std::uint32_t>::max()) result.lifetimes.push_back(lifetime);
        }
...
std::unique_ptr<IRenderGraph> create_render_graph(std::string& errorOut) {
    errorOut.clear();
    return std::make_unique<RenderGraphCore>();
}
