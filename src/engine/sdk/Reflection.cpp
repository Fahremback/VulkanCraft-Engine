...
public:
 bool register_type(const TypeInfo&t,std::string&e)override{if(!valid_type(t,e))return false;if(types_.count(t.name)||(!t.alias.empty()&&std::any_of(types_.begin(),types_.end(),[&](auto&p){return p.second.alias==t.alias;}))){e="reflection: duplicate type or alias";return false;}types_[t.name]=t;return true;}
