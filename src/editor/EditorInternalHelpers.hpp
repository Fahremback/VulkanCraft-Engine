...
// dir `${CMAKE_BINARY_DIR}/shaders`) to the absolute file actually opened and
// accepts only a shader basename. To make WHICH tree the editor actually
// executed auditable, every completely-read module is fingerprinted (FNV-1a
// 64 over the raw SPIR-V words) and the absolute path + hash are logged at boot. Certification
